...
test('cross-ledger recall de-duplicates the same commitment and keeps its due date', () => {
  const content = 'Aufgabe: Rechnung einreichen';
  const merged = mergeProactiveItems(
    [{ key: 'task-ledger:7', text: `Unterbrochener Auftrag: ${content}`, dueAtUtc: null, dedupeKey: normalizeProactiveContent(content) }],
    [{ key: 'capture:cap_7', text: `Offener Auftrag: ${content}`, dueAtUtc: '2026-07-18T00:00:00.000Z', dedupeKey: normalizeProactiveContent(content) }],
  );
  assert.equal(merged.length, 1);
  assert.equal(merged[0]?.key, 'task-ledger:7');
  assert.equal(merged[0]?.dueAtUtc, '2026-07-18T00:00:00.000Z');
});
