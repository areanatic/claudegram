...
function isWhisperSilenceHallucination(text: string, durationSec: number): boolean {
  const normalized = text.toLowerCase().replace(/[.!?,;:]+$/g, '').trim();
  if (durationSec < 1.5 && STRICT_SHORT_SILENCE_HALLUCINATIONS.has(normalized)) return true;
  if (durationSec < 0.8 && AMBIGUOUS_SHORT_UTTERANCES.has(normalized)) return true;
  return false;
}
 ...
 */
function detectLoopHallucination(text: string): boolean {
  const normalized = text.toLowerCase().replace(/[.,!?;:'"’…]+/g, ' ').trim();
  const tokens = normalized.split(/\s+/).filter(Boolean);
  if (tokens.length < 8) return false;

  for (let n = 1; n <= 4; n++) {
    if (tokens.length < n * 4) break;
    ...
    for (const [ng, count] of counts) {
      ...
      // hallucination.
      if (tokens.length < 10 && n === 1) continue;
      ...
      for (let offset = 0; offset < n; offset++) {
        ...
        for (let i = offset; i <= tokens.length - n; i += n) {
          if (tokens.slice(i, i + n).join(' ') === ng) {
            curr++;
            if (curr > maxConsecutive) maxConsecutive = curr;
          } else {
            curr = 0;
          }
        }
      }
      if (maxConsecutive >= 4 && coverage >= 0.5) return true;
    }
  }
  return false;
}
...
export async function transcribeFileWithLanguage(filePath: string, options?: TranscribeOptions): Promise<TranscribeResult> {
  ...
  let qualityFlag: VoiceQualityFlag = 'ok';
  ...
  if (qualityFlag !== 'ok') {
    console.warn(
      `[transcribeFileWithLanguage] quality-flag=${qualityFlag} ` +
