...
function setRow(id: number, fields: Record<string, unknown>): void {
  withDb((db) => {
    const sets = Object.keys(fields).map((k) => `${k} = @${k}`).join(', ');
    db.prepare(`UPDATE input_log SET ${sets} WHERE id = @id`).run({ ...fields, id });
  });
}
function getRow(id: number): Record<string, unknown> {
  return withDb((db) => db.prepare('SELECT * FROM input_log WHERE id = ?').get(id) as Record<string, unknown>);
}
...
(async () => {
  ...
  function insert(opts: {
    chatId: number;
    sessionKey: string;
    inputType?: 'text' | 'voice' | 'photo' | 'audio' | 'document';
    rawContent?: string | null;
    privacy?: 'public' | 'private';
    receivedAtMs: number; // offset from now
    resumeAttempts?: number;
    sideEffectAt?: string | null;
  }): number {
    ...
    const fields: Record<string, unknown> = { received_at: iso(opts.receivedAtMs) };
    if (opts.resumeAttempts != null) fields.resume_attempts = opts.resumeAttempts;
    if (opts.sideEffectAt !== undefined) fields.side_effect_tool_started_at = opts.sideEffectAt;
    setRow(id!, fields);
    return id!;
  }
  ...
  const firstStamp = getRow(S).side_effect_tool_started_at as string | null;
  check(!!firstStamp, 'side_effect_tool_started_at stamped on first mutating tool');
  check((getRow(S).tool_execution_names as string) === 'Bash', 'tool name recorded');
  markSideEffectStarted(S, 'Write');
  check((getRow(S).side_effect_tool_started_at as string) === firstStamp, 'timestamp stable (first tool wins)');
  check((getRow(S).tool_execution_names as string).split(',').sort().join(',') === 'Bash,Write', 'second tool appended');
  markSideEffectStarted(S, 'Bash'); // dedup
  check((getRow(S).tool_execution_names as string).split(',').filter((t) => t === 'Bash').length === 1, 'duplicate tool name deduped');
