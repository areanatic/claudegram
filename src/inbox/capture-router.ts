...
import { detectPlatform, isValidUrl, type Platform } from '../media/extract.js';
import { config } from '../config.js';
import { insertCapture, type CaptureInsert } from './captures-db.js';
import { enrichCaptureAsync } from './capture-enrichment.js';

const URL_REGEX = /https?:\/\/[^\s)]+/gi;

interface ClassifiedCapture {
  capture_type: CaptureInsert['capture_type'];
...
function extractFirstUrl(text: string | undefined | null): string | null {
  ...
  for (const m of matches) {
    if (isValidUrl(m)) return m;
  }
  return null;
}

function classify(ctx: Context): ClassifiedCapture | null {
  ...
  const fwd = (msg as unknown as { forward_origin?: unknown; forward_from?: unknown; forward_from_chat?: unknown }).forward_origin
    ?? (msg as unknown as { forward_from?: unknown }).forward_from
    ?? (msg as unknown as { forward_from_chat?: unknown }).forward_from_chat;
  if (fwd) {
    tags.push('forward');
    meta.forward = fwd;
  }
  ...
  if (msg.document) {
    tags.push('document');
    if (msg.document.mime_type) tags.push(msg.document.mime_type.split('/')[0]);
    return {
  ...
  if (url) {
    ...
    tags.push('url');
    if (platform !== 'unknown') tags.push(platform);
    return {
      capture_type: 'url',
      platform,
      source_url: url,
  ...
  tags.push('text');
  if (text.length < 20) tags.push('short'); // markiert kurze Texts für späteres Filtering, falls nötig
  return {
    capture_type: 'text',
    raw_text: text,
    raw_meta_json: Object.keys(meta).length ? JSON.stringify(meta) : null,
...
async function ackReaction(ctx: Context, chatId: number, messageId: number): Promise<void> {
  try {
    // grammY exposes setMessageReaction via ctx.api
    await (ctx.api as unknown as {
      setMessageReaction: (
        chat_id: number,
        message_id: number,
...
export async function captureRouter(ctx: Context, next: NextFunction): Promise<void> {
  try {
    const msg = ctx.message;
    ...
    const privacy: 'public' | 'private' = isPrivate(sessionKey) ? 'private' : 'public';
    ...
    if (id !== null) {
      ...
      enrichCaptureAsync(
        ctx,
        id,
        classified.capture_type,
        classified.telegram_file_id ?? null,
        classified.source_url ?? null,
      );
    }
  } catch (err) {
