...
import {
  ...
  markDropped,
  hasNewerDuplicate,
  hasLaterOpenVoiceRow,
  getRowExecutionState,
  incrementResumeAttempt,
} from './input-log.js';

const LONG_VOICE_DECOMPOSE_INSTRUCTION =
  'Dies ist ein langer transkribierter Sprach-Auftrag, der vorher in einen Timeout lief. ' +
 ...
 */
export async function tryAutoDispatch(args: AutoDispatchArgs): Promise<AutoDispatchOutcome> {
  const { api, chatId, sessionKey, rowId, rawContent, inputType, reason, statusMessageId } = args;
  if (!config.INPUT_AUTODISPATCH_ENABLED) return 'fallback';
  if (rowId == null || !rawContent.trim()) return 'fallback';
  ...
  if (statusMessageId != null) {
    try {
      await api.editMessageText(
        chatId,
        statusMessageId,
        '⏳ Das dauert etwas länger — ich mach das automatisch für dich fertig…',
      );
  ...
  try {
    ...
    const response = await queueRequest(sessionKey, prompt, (turnEpoch) =>
      sendToAgent(sessionKey, prompt, {
        turnEpoch,
