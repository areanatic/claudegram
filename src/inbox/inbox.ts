 ...
 */
export function buildInboxFilename(originalName: string): string {
 ...
 */
export function saveMetadata(metadata: InboxMetadata): void {
 ...
 */
export function readMetadata(filePath: string): InboxMetadata | null {
...
export function createInboxEntry(opts: {
  ...
  caption: string | null;
  senderId: number;
}): { metadata: InboxMetadata; destPath: string } {
...
export function routeFile(
  ...
  targetDir: string,
  newFilename?: string
): { newPath: string; metadata: InboxMetadata | null } {
 ...
 */
export function listInbox(): InboxMetadata[] {
  const inboxDir = getInboxDir();
  if (!fs.existsSync(inboxDir)) return [];

  const entries = fs.readdirSync(inboxDir, { withFileTypes: true });
  const results: InboxMetadata[] = [];

  for (const entry of entries) {
    // Skip metadata sidecars and directories
    if (entry.name.endsWith(METADATA_EXTENSION) || !entry.isFile()) continue;
