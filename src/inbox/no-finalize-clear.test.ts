/**
 * RI-23 regression — markDone/markProcessing must CLEAR a stale dropped_reason.
 *
 * Run: npx tsx src/inbox/no-finalize-clear.test.ts
 *
 * Bug (Phase-A forensics + Codex M-11 Q4, 2026-06-06): media handlers (photo/document/
 * transcribe) never marked their input-log row done. The catch-all finalizeIfOpen then
 * stamped dropped_reason='handler_no_finalize' on rows that were actually processed fine
 * → 34 false "handler_no_finalize" rows (measurement artefact, NOT data loss). Worse: even
 * once a handler DID call markDone, the old code did not clear dropped_reason, so a row that
 * raced finalizeIfOpen would keep the stale 'handler_no_finalize' tag despite success.
 *
 * Fix: markDone + markProcessing now set dropped_reason = NULL. This test proves:
 *   (1) finalizeIfOpen tags an unmarked row 'handler_no_finalize' (the artefact),
 *   (2) a later markDone CLEARS that tag (status=done, dropped_reason=NULL, response_sent_at set),
 *   (3) markProcessing also clears a stale reason,
...
const rowOf = (id: number) => db.prepare('SELECT status, dropped_reason, response_sent_at FROM input_log WHERE id = ?').get(id) as any;

function newRow(msgId: number) {
...
let threw = false;
try { m.markDone(null); m.markProcessing(null); m.markError(null, 'x'); } catch { threw = true; }
