...
mock.module('../claude/request-queue.js', {
  namedExports: {
    queueRequest: async (_sessionKey: string, _message: string, handler: (e: number) => Promise<unknown>) =>
      handler(1),
  },
});
...
/** Minimal grammY Api stub that records every sendMessage / editMessageText call. */
interface SentCall { chatId: number; text: string; opts: { message_thread_id?: number } }
function makeApiStub() {
  const sent: SentCall[] = [];
  const api = {
    sendMessage: async (chatId: number, text: string, opts: { message_thread_id?: number } = {}) => {
...
(async () => {
  ...
  // ── Case 1: FORUM topic — sessionKey "chatId:threadId" → message_thread_id ──
  {
    ...
    check(
      sent.every((c) => c.chatId === chatId),
      'forum: every send targets the originating chat',
    );
    // THE FIX: every fresh send carries the recovered thread id.
    check(
      sent.every((c) => c.opts.message_thread_id === threadId),
      'forum: EVERY api.sendMessage carries message_thread_id (lands in originating topic)',
    );
  ...
  // ── Case 2: PLAIN chat — sessionKey is the bare chatId → NO thread ──────────
  {
    ...
    check(
      sent.every((c) => c.chatId === chatId),
      'plain: every send targets the chat',
    );
    // THE NON-REGRESSION: no thread is ever attached for a plain chat.
    check(
      sent.every((c) => c.opts.message_thread_id === undefined),
      'plain: NO api.sendMessage carries message_thread_id (zero regression)',
    );
    check(
      sent.every((c) => Object.keys(c.opts).length === 0),
      'plain: send opts are byte-identical to old behavior ({})',
    );
  }
