...
const AUTO_PLATFORMS: Platform[] = ['youtube', 'tiktok', 'instagram'];
...
const CATEGORY_RULES: { category: string; pattern: RegExp }[] = [
  {
    category: 'AI/Tech',
    pattern:
      /\b(ai|llm|model|api|database|sql|nosql|github|code|software|app|machine.?learning|neural|gpt|claude|openai|anthropic|python|javascript|typescript|programming|developer|tech|tool|framework|agent|vector|embedding|rag|wiki)\b/i,
  },
  {
    category: 'Finanzen',
    pattern:
      /\b(steuer|finanz|kredit|bank|zins|notar|immobili|budget|geld|investment|aktien|dividende|wohnung|kauf|miete|hypothek|steuerbescheid|finanzamt|kaufvertrag|grundschuld)\b/i,
  },
  {
    category: 'Gesundheit',
    pattern:
      /\b(gesundheit|arzt|medizin|therapie|krankenhaus|diagnose|symptom|behandlung|sport|fitness|ern.?hrung|schlaf|psychiatrie|neurologie|dermatologie)\b/i,
  },
  {
    category: 'Bildung',
    pattern:
      /\b(erkl.{0,4}rt|tutorial|lernen|wissen|einfach.erkl|how.to|guide|kurs|schule|universit|studium|lektion)\b/i,
  },
  {
    category: 'News',
    pattern:
      /\b(news|nachrichten|aktuell|politik|wirtschaft|gesellschaft|bericht|journalist|breaking)\b/i,
  },
];
 ...
 */
export async function processLinkInbox(
  ctx: Context,
  url: string,
  sessionKey: string
): Promise<void> {
  ...
  const editAck = async (text: string) => {
    if (!ackMsgId) return;
    try {
      await ctx.api.editMessageText(chatId, ackMsgId, text, { parse_mode: undefined });
    } catch {
      // fallback: send new message
      try { await ctx.reply(text, { parse_mode: undefined }); } catch { /* ignore */ }
    }
  };
  ...
  try {
    result = await Promise.race([
      ...
      new Promise<never>((_, reject) =>
        setTimeout(() => reject(new Error('Transcript timeout nach 90s')), 90_000)
      ),
    ]);
  } catch (err) {
