...
const ctx = { sessionKey: '1', telegramCtx: {} } as unknown as Parameters<typeof bridge.dirigentDispatchTool>[0];
...
let taskId = '';
{
  ...
  const row = inspect.prepare(`SELECT * FROM tasks WHERE id=?`).get(taskId) as Record<string, unknown>;
  ...
  const ev = inspect.prepare(`SELECT COUNT(*) c FROM events WHERE task_id=? AND event_type='enqueued'`).get(taskId) as { c: number };
...
// ── 2. budget mapping per task_type ───────────────────────────────────────────
{
  ...
  const row = inspect.prepare(`SELECT budget_usd_max FROM tasks WHERE id=?`).get(id) as { budget_usd_max: number };
...
// ── 3. status: still-pending, unknown, then finished writeback ─────────────────
{
  ...
  check((pendingRes.content[0].text as string).includes('still pending'), 'status reports still pending');

  const unknownRes = await status.handler({ task_id: 'nope-does-not-exist' }, {});
  check((unknownRes.content[0].text as string).includes('No Dirigent task found'), 'status reports unknown id');
  ...
  const dt = doneRes.content[0].text as string;
...
// ── 3b. SECURITY (Codex P1): status must NOT read foreign / non-bridge tasks ───
{
  ...
  const w = new Database(DB_PATH);
  w.prepare(
    `INSERT INTO tasks (id, prompt, task_type, status, source_type, actor, channel, privacy, result_json)
     VALUES ('foreign-omi-001','secret','adhoc','needs_review','omi','automation','privat','private',?)`
  ).run(JSON.stringify({ result: 'SECRET-OMI-RESULT' }));
  w.close();
