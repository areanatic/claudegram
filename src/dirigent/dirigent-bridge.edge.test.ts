...
const ctx = { sessionKey: 'edge-1', telegramCtx: {} } as unknown as Parameters<typeof bridge.dirigentDispatchTool>[0];
...
const taskCount = () => (inspect.prepare(`SELECT COUNT(*) c FROM tasks`).get() as { c: number }).c;
const eventCount = () => (inspect.prepare(`SELECT COUNT(*) c FROM events`).get() as { c: number }).c;
const idFromText = (t: string): string => {
...
{
  const expected: Record<string, number> = { adhoc: 0.20, 'code-audit': 0.40, 'morning-brief': 0.30, 'ask-chat': 0.05 };
  for (const [tt, budget] of Object.entries(expected)) {
    const res = await dispatch.handler({ prompt: `do ${tt}`, task_type: tt as 'adhoc', priority: undefined }, {});
    const id = idFromText(res.content[0].text as string);
    check(id !== '', `dispatch ${tt} returned id`);
    const row = inspect.prepare(`SELECT budget_usd_max, permission_profile, priority FROM tasks WHERE id=?`).get(id) as { budget_usd_max: number; permission_profile: string; priority: number };
...
// ── 2. priority bounds 1 and 10 accepted + persisted ───────────────────────────
{
  ...
  const id1 = idFromText(r1.content[0].text as string);
  check((inspect.prepare(`SELECT priority FROM tasks WHERE id=?`).get(id1) as { priority: number }).priority === 1, 'priority=1 (high bound) persisted');
  const r10 = await dispatch.handler({ prompt: 'p10', task_type: 'adhoc', priority: 10 }, {});
  const id10 = idFromText(r10.content[0].text as string);
  check((inspect.prepare(`SELECT priority FROM tasks WHERE id=?`).get(id10) as { priority: number }).priority === 10, 'priority=10 (low bound) persisted');
}
...
// ── 4. status: NON-terminal 'running' → "still running" ────────────────────────
{
  ...
  check((res.content[0].text as string).includes('still running'), 'status non-terminal running → "still running"');
}

// ── 5. status: every TERMINAL status surfaced correctly ────────────────────────
{
  const mk = (id: string, st: string, extra = '') => {
    const w = new Database(DB_PATH);
  ...
  const done = (await status.handler({ task_id: 'edge-done' }, {})).content[0].text as string;
  check(done.includes('done') && done.includes('DONE-PAYLOAD'), 'terminal done surfaces result');
  const canc = (await status.handler({ task_id: 'edge-cancelled' }, {})).content[0].text as string;
  check(canc.includes('cancelled') && canc.includes('CANCEL-PAYLOAD'), 'terminal cancelled surfaces result');
  const rej = (await status.handler({ task_id: 'edge-rejected' }, {})).content[0].text as string;
  check(rej.includes('rejected') && rej.includes('REJECT-PAYLOAD'), 'terminal rejected surfaces result');
  const stuck = (await status.handler({ task_id: 'edge-stuck' }, {})).content[0].text as string;
  check(stuck.includes('stuck') && stuck.includes('STUCK-PAYLOAD'), 'terminal stuck surfaces result');
  const failed = (await status.handler({ task_id: 'edge-failed' }, {})).content[0].text as string;
...
// ── 6. failed with NULL error_message → "unknown error" ────────────────────────
{
  ...
  w.close();
  const txt = (await status.handler({ task_id: 'edge-failed-noerr' }, {})).content[0].text as string;
...
// ── 7. malformed result_json (not JSON) → graceful, raw shown, no crash ─────────
{
  ...
  w.close();
  const txt = (await status.handler({ task_id: 'edge-malformed' }, {})).content[0].text as string;
...
// ── 8. result_json WITHOUT a `result` key → falls back to JSON.stringify(parsed) ─
{
  ...
  w.close();
  const txt = (await status.handler({ task_id: 'edge-nokey' }, {})).content[0].text as string;
...
// ── 9. result_json with a NON-STRING `result` (object) → stringify fallback ─────
{
  ...
  w.close();
  const txt = (await status.handler({ task_id: 'edge-objresult' }, {})).content[0].text as string;
...
// ── 10. HUGE result (>3000 chars) → truncated with … ───────────────────────────
{
  ...
  w.close();
  const txt = (await status.handler({ task_id: 'edge-huge' }, {})).content[0].text as string;
...
// ── 11. NULL result_json on terminal non-failed → "(no result captured)" ───────
{
  ...
  w.close();
  const txt = (await status.handler({ task_id: 'edge-noresult' }, {})).content[0].text as string;
...
// ── 12. SQLITE_BUSY resilience: competing write lock held by a SEPARATE OS PROCESS ──
{
  ...
  const landed = inspect.prepare(`SELECT COUNT(*) c FROM tasks WHERE id=? AND prompt='busy-test'`).get(busyId) as { c: number };
