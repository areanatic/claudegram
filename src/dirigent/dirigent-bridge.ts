...
function dirigentDbPath(): string {
...
const TASK_PROFILES: Record<string, { profile: string; budget: number }> = {
...
const TASK_TYPES = ['adhoc', 'code-audit', 'morning-brief', 'ask-chat'] as const;
...
export function dirigentDispatchTool(toolsCtx: McpToolsContext) {
  return tool(
    'dirigent_dispatch',
    'Dispatch ONE background sub-task to the Dirigent worker queue when a request is long-running ' +
    ...
    async ({ prompt, task_type, priority }) => {
      ...
      try {
        ...
        const insert = db.prepare(
          `INSERT INTO tasks (id, prompt, task_type, priority, channel, privacy, source_type, actor, permission_profile, budget_usd_max, depth)
        ...
        return {
          content: [{ type: 'text' as const, text: `Dirigent dispatch error: ${error instanceof Error ? error.message : String(error)}` }],
          isError: true,
        };
      } finally {
...
export function dirigentStatusTool(_toolsCtx: McpToolsContext) {
  return tool(
    'dirigent_status',
    'Check the status / collect the result of a background sub-task previously created with ' +
    ...
    async ({ task_id }) => {
      ...
      try {
        ...
        const row = db.prepare(
          `SELECT status, result_json, error_message, cost_usd FROM tasks
           WHERE id = ? AND source_type = 'telegram' AND actor = 'arash'
                 AND channel = 'nexus-dev' AND privacy = 'public'`
        ).get(task_id) as { status: string; result_json: string | null; error_message: string | null; cost_usd: number | null } | undefined;

        if (!row) {
          return { content: [{ type: 'text' as const, text: `No Dirigent task found with id "${task_id}".` }] };
        }
        if (!TERMINAL.has(row.status)) {
          return { content: [{ type: 'text' as const, text: `Task ${task_id} is still ${row.status}. The worker runs ~every 15 min — check again shortly.` }] };
        }
        if (row.status === 'failed') {
          return { content: [{ type: 'text' as const, text: `Task ${task_id} failed: ${row.error_message ?? 'unknown error'}.` }] };
        }
        let result = '(no result captured)';
        if (row.result_json) {
          try {
            const parsed = JSON.parse(row.result_json) as { result?: unknown };
        ...
        return { content: [{ type: 'text' as const, text: `🎛 Task ${task_id} → ${row.status}${cost}:\n\n${truncated}` }] };
      } catch (error) {
        return {
          content: [{ type: 'text' as const, text: `Dirigent status error: ${error instanceof Error ? error.message : String(error)}` }],
          isError: true,
        };
      } finally {
        try { db?.close(); } catch { /* swallow */ }
