...
export function registerTranscribePrompt(
  chatId: number,
  userId: number,
  promptMessageId: number,
  now: number = Date.now(),
 ...
 * swept on lookup.
 */
export function isFreshTranscribeReply(
  chatId: number,
  userId: number,
 ...
 */
export function takeFreshTranscribeReply(
  chatId: number,
  userId: number,
  replyToMessageId: number | null | undefined,
  now: number = Date.now(),
): boolean {
  if (!isFreshTranscribeReply(chatId, userId, replyToMessageId, now)) return false;
  pending.delete(key(chatId, replyToMessageId as number));
  return true;
}

/** Test-only: clear the registry between cases. */
export function __resetTranscribePendingForTest(): void {
  pending.clear();
}
