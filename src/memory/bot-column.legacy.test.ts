...
const { saveMemory, searchMemoryReadOnly, recentMemoriesReadOnly, closeMemoryDb, DEFAULT_TRUSTED_PRIVATE_SOURCES } =
...
// ── saveMemory pre-migration fallback (privacy + bot params ignored, no crash) ─
{
  ...
  check(!cols.some(c => c.name === 'privacy') && !cols.some(c => c.name === 'bot'),
    'sanity: DB really has NEITHER privacy NOR bot column');
  const row = inspect.prepare(`SELECT content, source, project FROM memories WHERE content LIKE ?`).get(`%LEGACY would-be%`) as { content: string; source: string; project: string };
...
// IS a robustness gap: on a pre-migration DB the keyword search tool silently yields zero.
{
  ...
  check(hits.length === 0,
    `GAP: searchMemoryReadOnly returns [] on pre-migration DB (m.privacy SELECT throws → catch → []) (got ${hits.length})`);
...
// public" posture — exactly why the privacy migration must run before private mode is trusted).
{
  ...
  check(rec.some(h => h.content.includes('LEGACY would-be-private')),
    'pre-migration: recent surfaces the "private"-intended row (no privacy column to gate — SURPRISING-but-documented)');
