...
const { saveMemory, botId, DEFAULT_TRUSTED_PRIVATE_SOURCES, searchMemoryReadOnly, closeMemoryDb } =
...
// ── 6. source col accepts non-allowlist free-text on WRITE; READ still gates it ─
{
  ...
  const disk = inspect.prepare(`SELECT source, privacy FROM memories WHERE content LIKE ?`).get(`%ROGUE%`) as { source: string; privacy: string };
