...
export interface ProactiveItem {
  ...
  text: string;
  dueAtUtc: string | null;
  /** Normalized source content for cross-ledger display de-duplication. */
  dedupeKey?: string;
}

export function normalizeProactiveContent(value: string): string {
  return value.toLocaleLowerCase('de-DE').replace(/\s+/g, ' ').trim();
}

/** Keep the richer first source while retaining a due date from a duplicate. */
export function mergeProactiveItems(...groups: readonly ProactiveItem[][]): ProactiveItem[] {
  const merged = new Map<string, ProactiveItem>();
  for (const item of groups.flat()) {
    const identity = item.dedupeKey || item.key;
    const existing = merged.get(identity);
    if (!existing) {
      merged.set(identity, item);
    } else if (!existing.dueAtUtc && item.dueAtUtc) {
      merged.set(identity, { ...existing, dueAtUtc: item.dueAtUtc });
    }
  }
...
export class CaptureLedger {
  ...
  pendingForSession(sessionKey: string, now = new Date()): ProactiveItem[] {
    ...
    return rows.map((row) => ({
      ...
      dedupeKey: normalizeProactiveContent(row.content),
    }));
  }
