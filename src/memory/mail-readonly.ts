...
import { readMemoryPolicyFromEnv, type MemoryRetrievalPolicy } from './nexus-memory.js';
...
export interface MailOverview {
  ...
  recent: Array<{ date_utc: string; from_domain: string; importance: string; tier: string }>;
}
 ...
 */
export function mailOverviewReadOnly(recent = 0): MailOverview | null {
  ...
  try {
    ...
    const totals = conn
      .prepare(
        `SELECT COUNT(*) AS c, MIN(date_utc) AS mn, MAX(date_utc) AS mx
         FROM messages WHERE from_domain <> ?`,
      )
      .get(ANCHOR_DOMAIN) as { c: number; mn: string | null; mx: string | null };

    const impRows = conn
      .prepare(
        `SELECT importance_hint AS k, COUNT(*) AS c FROM messages
         WHERE from_domain <> ? GROUP BY importance_hint`,
      )
      .all(ANCHOR_DOMAIN) as Array<{ k: string | null; c: number }>;

    const tierRows = conn
      .prepare(
        `SELECT privacy_tier AS k, COUNT(*) AS c FROM messages
    ...
    const domainRows = conn
      ...
      .all(ANCHOR_DOMAIN, TOP_DOMAINS) as Array<{ d: string; c: number }>;

    let outboxRows: Array<{ k: string; c: number }> = [];
    try {
      outboxRows = conn
    ...
    let recentList: MailOverview['recent'] = [];
    if (recentN > 0) {
      const rawRecent = conn
        ...
        .all(ANCHOR_DOMAIN, recentN) as Array<{
          date_utc: string; from_domain: string; importance_hint: string | null; privacy_tier: string;
        }>;
    ...
    const toMap = (rows: Array<{ k: string | null; c: number }>): Record<string, number> => {
      const m: Record<string, number> = {};
      for (const r of rows) m[r.k ?? 'n/a'] = r.c;
      return m;
    };
...
function formatOverview(o: MailOverview): string {
 ...
 */
export function mailOverviewTool(toolsCtx: McpToolsContext) {
  ...
  return tool(
    ...
    async ({ recent }) => {
      try {
        if (!isOperator) {
          return { content: [{ type: 'text' as const, text: 'Mail overview is not available in this bot context.' }] };
        }
        if (isPrivate(toolsCtx.sessionKey)) {
          return { content: [{ type: 'text' as const, text: 'Mail overview is unavailable while /private is on — turn /private off to query mail counts.' }] };
        }
        const o = mailOverviewReadOnly(recent ?? 0);
        if (o === null) {
          return { content: [{ type: 'text' as const, text: 'Mail overview unavailable.' }], isError: true };
        }
        return { content: [{ type: 'text' as const, text: formatOverview(o) }] };
      } catch (error) {
        ...
        return {
          content: [{ type: 'text' as const, text: 'Mail overview unavailable.' }],
          isError: true,
        };
      }
    },
