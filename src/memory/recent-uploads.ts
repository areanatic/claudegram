...
function sidecarPath(workingDir: string): string {
  return path.join(workingDir, SIDECAR_DIR, SIDECAR_FILE);
}

function readSidecar(workingDir: string): SidecarShape {
  try {
    const file = sidecarPath(workingDir);
    if (!fs.existsSync(file)) return { uploads: [] };
    const raw = fs.readFileSync(file, 'utf-8');
    if (!raw.trim()) return { uploads: [] };
    const parsed = JSON.parse(raw);
    if (parsed && Array.isArray(parsed.uploads)) {
      return { uploads: parsed.uploads.slice(0, MAX_ENTRIES) as UploadEntry[] };
    }
    return { uploads: [] };
  } catch {
    return { uploads: [] };
...
function writeSidecar(workingDir: string, data: SidecarShape): void {
  try {
...
export function recordUpload(
  workingDir: string,
  entry: { path: string; caption?: string; ts?: string }
): void {
  const data = readSidecar(workingDir);
  const fullEntry: UploadEntry = {
    path: entry.path,
