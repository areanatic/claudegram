 ...
 */
export function occupancyTokens(u) {
 ...
 */
export function classifyContextPressure(usedTokens, contextWindow) {
    if (!Number.isFinite(contextWindow) || contextWindow <= 0)
        return 'none';
    if (!Number.isFinite(usedTokens) || usedTokens < 0)
        return 'none';
    const ratio = usedTokens / contextWindow;
    if (ratio >= CONTEXT_ROTATE_HARD)
        return 'rotated';
    if (ratio >= CONTEXT_ROTATE_WARN)
        return 'warned';
    return 'none';
}
 ...
 */
export function effectiveWindowTokens(model) {
 ...
 */
export function exceedsBootWindow(sizeBytes, model) {
 ...
 */
export function isContextOverflowSentinel(text) {
    if (!text)
        return false;
