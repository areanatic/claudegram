 ...
 */
function resolveWorkingDirectory(storedPath) {
...
class SessionManager {
            ...
            for (const dir of projectDirs) {
                ...
                if (fs.existsSync(sessionFile)) {
                    ...
                    if (exceedsBootWindow(size, model)) {
                        console.log(`[SessionRotation] JSONL context-full at boot: ${Math.round(size / 1024 / 1024)}MB ` +
    ...
    createSession(sessionKey, workingDirectory, conversationId) {
    ...
    updateActivity(sessionKey, messagePreview) {
    ...
    setWorkingDirectory(sessionKey, directory) {
        const existing = this.sessions.get(sessionKey);
        if (existing) {
    ...
    resumeSession(sessionKey, conversationId) {
    ...
    getSessionHistory(sessionKey, limit = 5) {
        return sessionHistory.getHistory(sessionKey, limit);
    }
    setClaudeSessionId(sessionKey, claudeSessionId) {
        const session = this.sessions.get(sessionKey);
        if (!session)
            return;
