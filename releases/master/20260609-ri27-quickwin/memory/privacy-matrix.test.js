...
function seed() {
  ...
  `);
    ...
    const ins = db.prepare(`INSERT INTO memories (type, content, source, project, privacy, created_at)
...
const publicPolicy = { scope: 'public', trustedPrivateSources: trusted };
const selfPrivatePolicy = { scope: 'self_private', trustedPrivateSources: trusted };
const operatorAllPolicy = { scope: 'operator_all', trustedPrivateSources: trusted };
const has = (rows, marker) => rows.some(r => r.content.includes(marker));
const markers = (rows) => ['PUB_ONE', 'PUB_TWO', 'PRIV_TRUST', 'PRIV_UNTRUST'].filter(m => has(rows, m)).sort();
// ── searchMemoryReadOnly (FTS5 path) ──────────────────────────────────────────
{
    ...
    check(markers(privateModeEffective).join(',') === markers(pub).join(','), 'search: /private-on effective policy == public row-set (trusted-private downgraded away)');
}
...
//# sourceMappingURL=privacy-matrix.test.js.map