...
const { saveMemory, searchMemoryReadOnly, recentMemoriesReadOnly, closeMemoryDb } = mem;
...
// ── saveMemory still works (privacy branch, bot absent) — even with explicit bot param ─
{
    ...
    const row = inspect.prepare(`SELECT privacy FROM memories WHERE content LIKE ?`).get(`%NOBOT public%`);
    ...
    const row2 = inspect.prepare(`SELECT privacy FROM memories WHERE content LIKE ?`).get(`%NOBOT private%`);
...
//# sourceMappingURL=bot-column.nobot.test.js.map