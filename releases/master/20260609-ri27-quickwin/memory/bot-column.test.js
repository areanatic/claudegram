...
function seed() {
  ...
  `);
    ...
    const ins = db.prepare(`INSERT INTO memories (type, content, source, project, privacy, bot, created_at)
...
const { saveMemory, searchMemoryReadOnly, recentMemoriesReadOnly, botId, DEFAULT_TRUSTED_PRIVATE_SOURCES, closeMemoryDb } = mem;
...
const selfPrivate = { scope: 'self_private', trustedPrivateSources: trusted };
const publicPol = { scope: 'public', trustedPrivateSources: trusted };
{
...
// ── 4. SAFETY: bot column does NOT change the privacy matrix ───────────────────
{
    ...
    check(!has(pub, 'PRIV_TRUST') && !has(pub, 'PRIV_UNTRUST'), 'safety/public: NO private row leaks regardless of bot (master-bot/family-bot private both hidden)');
    ...
    check(!has(sp, 'PRIV_UNTRUST'), 'safety/self_private: untrusted-private STILL hidden even though saved by a different bot');
}
// cleanup
try {
    inspect.close();
...
//# sourceMappingURL=bot-column.test.js.map