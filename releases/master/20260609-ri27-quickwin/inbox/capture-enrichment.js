...
async function downloadTelegramFile(ctx, telegramFileId, destExt) {
    const file = await ctx.api.getFile(telegramFileId);
    if (!file.file_path)
        throw new Error('No file_path from Telegram getFile');
 ...
 */
export async function enrichVoiceCapture(ctx, captureId, telegramFileId) {
    let dest = null;
    try {
 ...
 */
export async function enrichPhotoCapture(ctx, captureId, telegramFileId) {
 ...
 */
export async function enrichUrlCapture(captureId, sourceUrl) {
    try {
 ...
 */
export function enrichCaptureAsync(ctx, captureId, captureType, telegramFileId, sourceUrl) {
...
//# sourceMappingURL=capture-enrichment.js.map