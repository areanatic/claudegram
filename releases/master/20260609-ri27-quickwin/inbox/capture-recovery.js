...
function findQueuedRecoverable() {
    const conn = getDb();
    try {
        const placeholders = RECOVERABLE_TYPES.map(() => '?').join(',');
        return conn
            .prepare(`SELECT id, chat_id, message_id, message_thread_id, capture_type, telegram_file_id,
         ...
         ORDER BY created_at DESC`)
            .all(botId(), ...RECOVERABLE_TYPES, `-${RECOVERY_WINDOW_HOURS} hours`);
    }
    finally {
        conn.close();
    }
}
function markProcessed(id, transcript, tagSuffix) {
    const conn = getDb();
    try {
        conn
...
async function transcribeVoice(bot, telegramFileId) {
 ...
 */
async function visionPhoto(bot, telegramFileId) {
 ...
 */
async function extractUrlSummary(url) {
    try {
...
async function deliverToChat(bot, chatId, cap, transcript) {
 ...
 */
export async function runCaptureRecovery(bot) {
    ...
    console.log(`[CaptureRecovery] Found ${queued.length} queued (recovering ${work.length}, skipping ${skipped} due to per-chat-cap=${RECOVERY_PER_CHAT})`);
    for (const cap of work) {
        try {
            ...
            try {
                await bot.api.sendMessage(Number(cap.chat_id), `⚠️ Capture #${cap.id} (${cap.capture_type} vom ${cap.created_at.slice(0, 16)}) konnte nicht recovered werden: ${msg}`, cap.message_thread_id != null ? { message_thread_id: cap.message_thread_id } : {});
            }
            catch { /* best-effort only */ }
        }
    }
}
//# sourceMappingURL=capture-recovery.js.map