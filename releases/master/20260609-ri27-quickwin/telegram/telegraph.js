 ...
 */
export async function initTelegraph() {
    try {
        ...
        if (fs.existsSync(accountFile)) {
            ...
            const result = raw ? telegraphAccountSchema.safeParse(raw) : { success: false };
            if (result.success) {
        ...
        telegraphAccount = {
            ...
            short_name: json.result.short_name || 'Nexusgram'
        };
        // Save for future use
 ...
 */
export function shouldUseTelegraph(content, sessionKey) {
 ...
 */
async function createPageWithUuidTitle(token, displayTitle, content) {
 ...
 */
export async function createTelegraphPage(title, markdown) {
    if (!telegraphAccount) {
        await initTelegraph();
    }
    if (!telegraphAccount) {
    ...
    try {
        ...
        const page = await createPageWithUuidTitle(telegraphAccount.access_token, title, content);
        return page.url;
    }
    catch (error) {
 ...
 */
export async function createTelegraphFromFile(filePath) {
    try {
