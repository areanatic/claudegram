...
class SessionHistory {
    ...
    saveSession(sessionKey, conversationId, projectPath, lastMessagePreview = '', claudeSessionId, incrementMessageCount = true) {
        ...
        const existingIndex = history.findIndex((entry) => entry.conversationId === conversationId);
    ...
    getHistory(sessionKey, limit = 5) {
        const history = this.data.sessions[sessionKey] || [];
        return history.slice(0, limit);
    }
    getLastSession(sessionKey) {
        const history = this.data.sessions[sessionKey];
        return history?.[0];
    }
    getSessionByConversationId(sessionKey, conversationId) {
        const history = this.data.sessions[sessionKey] || [];
    ...
    updateLastMessage(sessionKey, conversationId, preview) {
        const history = this.data.sessions[sessionKey];
        if (!history)
            return;
    ...
    updateClaudeSessionId(sessionKey, conversationId, claudeSessionId) {
        const history = this.data.sessions[sessionKey];
        if (!history)
            return;
        const entry = history.find((e) => e.conversationId === conversationId);
