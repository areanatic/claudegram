...
import { classifyContextPressure, occupancyTokens, isContextOverflowSentinel, shouldRetryAfterOverflow } from './context-pressure.js';
...
import { createAgentTimer, recordMessage, formatDuration, getElapsedMs, getTimingReport, } from '../utils/agent-timer.js';
 ...
 */
export const CLAUDE_CANCEL_SENTINEL_TEXT = '✅ Successfully cancelled - no tools or agents in process.';
/**
...
async function interruptForToolBudget(q, sessionKey) {
    if (!q)
        return;
        ...
        await Promise.race([
            ...
            new Promise((resolve) => {
                timeoutHandle = setTimeout(() => {
                    console.warn(`[ToolBudget] ${sessionKey} interrupt() exceeded ` +
...
export class StaleTurnError extends Error {
    ...
    turnEpoch;
    name = 'StaleTurnError';
    constructor(sessionKey, turnEpoch) {
 ...
 */
export function assertTurnIsCurrent(sessionKey, turnEpoch) {
...
const chatUsageCache = new Map();
export function getCachedUsage(sessionKey) {
...
function extractButtons(text) {
    ...
    if (buttons.length < 2)
        return { text, buttons: [] }; // need at least 2 options
 ...
 */
function stripReasoningSummary(text) {
...
function getLogLevel() {
    return config.CLAUDE_SDK_LOG_LEVEL;
}
function logAt(level, message, data) {
...
function getPermissionMode(command) {
 ...
 */
function logDangerousModeOperation(sessionKey, operation, details) {
    if (!config.DANGEROUS_MODE)
        return;
...
function buildContextAvailabilityPrompt(sessionKey, currentInputLogRowId) {
    const lines = ['', '## Context Availability (this turn, runtime-checked):'];
    // 1. Today's daily log
    ...
    try {
        if (fs.existsSync(NEXUS_OMI_AUDIT_DIR)) {
            ...
            if (latest) {
                ...
                if (m)
                    lines.push(`- OMI: latest pull-log ${m[1]}`);
            }
        }
    }
    catch {
...
export async function sendToAgent(sessionKey, message, options = {}) {
    ...
    const isStillOwnerTurn = () => myTurnEpoch === undefined || isCurrentTurnEpoch(sessionKey, myTurnEpoch);
    try {
                    ...
                    hooks: [async (input) => {
                            ...
                            try {
                                ...
                                if (userMsgs.length > 0) {
                                    ...
                                    saveMemory(synthesis, 'semantic', config.BOT_MEMORY_PROJECT || 'nexus', 'precompact,telegram', 'nexusgram', isPrivate(sessionKey) ? 'private' : 'public');
                                }
                            }
                            catch {
        ...
        const verboseHooks = config.LOG_AGENT_HOOKS
            ? {
                PreToolUse: [{
                        hooks: [async (input) => {
        ...
        const SELF_MANAGEMENT_CMD = /\b(launchctl|kickstart|bootout|killall|pkill|shutdown|reboot|halt)\b/i;
        ...
        const killIsProcessMgmt = (cmd) => {
            ...
            while ((m = re.exec(cmd)) !== null) {
                ...
                if (!allJobControl)
                    return true; // any PID/$VAR/$()/`` target → process mgmt → DENY
            }
            return false;
        };
        const securityPreToolUse = {
            hooks: [async (input) => {
                    const i = input;
                    if (i.tool_name === 'Bash') {
                        ...
                        if (SELF_MANAGEMENT_CMD.test(cmd) || killIsProcessMgmt(cmd)) {
                            ...
                            return {
                                hookSpecificOutput: {
                                    ...
                                    permissionDecisionReason: 'Denied by the Akt-A4 security guard: this bot must never run ' +
        ...
        const isMutatingTool = (name) => MUTATING_TOOLS.has(name) || MUTATING_MCP_SUFFIXES.some((s) => name.endsWith(s));
        const sideEffectPreToolUse = {
            hooks: [async (input) => {
                    const i = input;
        ...
        const hooks = LOG_LEVELS[getLogLevel()] >= LOG_LEVELS.verbose
            ? {
                ...preCompactHook,
                ...verboseHooks,
        ...
        const contextAvailabilityContext = buildContextAvailabilityPrompt(sessionKey, options.currentInputLogRowId ?? null);
        const queryOptions = {
            cwd,
            tools: toolsOption,
            ...
            settingSources: config.BOT_SETTING_SOURCES,
            ...
            ? new AgentWatchdog({
                    ...
                    if (!watchdogUserWarningSent &&
                        ...
                        if (chatId !== undefined) {
                            ...
                            ctx.api
                                ...
                                .catch((err) => {
                                logAt('basic', `[Watchdog] Failed to notify user via Telegram: ${err instanceof Error ? err.message : String(err)}`);
                            });
                        }
                    }
                },
                onTimeout: () => {
                    ...
                    if (liveContexts.length === 0) {
                        logAt('basic', `[Claude] WATCHDOG: RequestContext already terminal for ${sessionKey} — ` +
        ...
        responseLoop: for await (const responseMessage of response) {
            ...
            if (responseMessage.type === 'assistant') {
                ...
                const stepUsage = responseMessage.message.usage;
                if (stepUsage) {
                    const occ = (stepUsage.input_tokens ?? 0) +
                        (stepUsage.cache_read_input_tokens ?? 0) +
                        (stepUsage.cache_creation_input_tokens ?? 0) +
                        (stepUsage.output_tokens ?? 0);
                    if (occ > maxWindowTokens)
                        maxWindowTokens = occ;
                }
                const stepModel = responseMessage.message.model;
                if (stepModel)
                    observedModel = stepModel;
                for (const block of responseMessage.message.content) {
                    ...
                    if (block.type === 'text') {
                        ...
                        if (toolsUsed.length > maxToolsThisTurn) {
                            watchdog?.stop();
                            console.warn(`[Claude] TOOL BUDGET EXCEEDED: ${toolsUsed.length}/${maxToolsThisTurn} ` +
                ...
                if (responseMessage.subtype === 'compact_boundary') {
                    ...
                    try {
                        ...
                        console.error('[Claude] MCP-INIT log error:', e);
                    }
                }
                else if (responseMessage.subtype === 'status') {
                    const statusMsg = responseMessage;
                ...
                const summaryId = 'tool_use_id' in responseMessage && typeof responseMessage.tool_use_id === 'string'
                    ? responseMessage.tool_use_id
                    : undefined;
                ...
                const resultMsg = responseMessage;
                if (resultMsg.modelUsage) {
                    ...
                    if (modelKey && resultMsg.modelUsage[modelKey]) {
                        ...
                        logAt('basic', `[Metrics] cache_read=${_cr} cache_creation=${_cc} ratio=${_ratio.toFixed(3)} ` +
    ...
    if (fullText && !abortController?.signal.aborted) {
        ...
        logConversationTurn(config.BOT_NAME, prompt, fullText, isPrivate(sessionKey) ? 'private' : 'public');
    }
...
export async function sendLoopToAgent(sessionKey, message, options = {}) {
    const { onProgress, abortController, maxIterations = config.MAX_LOOP_ITERATIONS, onIterationComplete, } = options;
    const session = sessionManager.getOrResumeSession(sessionKey);
    if (!session) {
        throw new Error('No active session. Use /project to set working directory.');
    }
...
export function clearConversation(sessionKey) {
    conversationHistory.delete(sessionKey);
    chatSessionIds.delete(sessionKey);
 ...
 */
export function forgetChatSession(sessionKey) {
    chatSessionIds.delete(sessionKey);
}
/**
 ...
 */
export function maybeRotateAfterContextPressure(sessionKey, usage) {
    if (!usage)
        return 'none';
 ...
 */
export function discardCancelledTurnState(sessionKey) {
...
export function setModel(sessionKey, model) {
    chatModels.set(sessionKey, model);
}
export function getModel(sessionKey) {
...
export function clearModel(sessionKey) {
...
const quietChats = new Set();
export function setQuiet(sessionKey, quiet) {
    if (quiet)
        quietChats.add(sessionKey);
    else
        quietChats.delete(sessionKey);
}
export function isQuiet(sessionKey) {
    return quietChats.has(sessionKey);
}
export function isDangerousMode() {
