 ...
 */
function buildFollowUpKeyboard(buttons) {
    if (!config.FOLLOWUP_BUTTONS_ENABLED)
        return undefined;
    if (!buttons || buttons.length < 2)
        return undefined;
    const rows = [];
    for (let i = 0; i < buttons.length; i += 2) {
        rows.push(buttons.slice(i, i + 2).map((label, j) => ({
            text: label,
 ...
 */
export async function sendFollowUpButtons(ctx, sessionKey, responseText, buttons) {
    try {
        if (!buttons || buttons.length < 2)
            return;
 ...
 */
export async function dismissFollowUpButtons(ctx, sessionKey) {
 ...
 */
export async function handleFollowUpCallback(ctx) {
    const data = ctx.callbackQuery?.data;
    if (!data || !data.startsWith('followup:'))
        return;
...
//# sourceMappingURL=followup-buttons.js.map