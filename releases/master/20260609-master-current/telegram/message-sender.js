...
import { getSpinnerFrame, getToolIcon, renderStatusLine, extractToolDetail, TOOL_ICONS, getAnimationEmoji, getAnimationText, } from './terminal-renderer.js';
 ...
 */
function splitOnSplitMarker(text) {
...
export class MessageSender {
     ...
     */
    async sendDocument(ctx, filePath, caption) {
        try {
     ...
     */
    async sendMarkdownFile(ctx, filePath, options = {}) {
     ...
     */
    updateToolOperation(sessionKey, toolName, input, ctx) {
        const state = this.streamStates.get(sessionKey);
        if (!state || !state.terminalMode)
            return;
     ...
     */
    updateBackgroundTask(sessionKey, taskName, status) {
        const state = this.streamStates.get(sessionKey);
        if (!state || !state.terminalMode)
            return;
    ...
    async flushTerminalUpdate(ctx, state) {
        ...
        try {
            await ctx.api.editMessageText(state.chatId, state.messageId, displayContent, { parse_mode: undefined });
    ...
    async finishStreaming(ctx, finalContent) {
        const keyInfo = getSessionKeyFromCtx(ctx);
        if (!keyInfo) {
    ...
    async cancelStreaming(ctx) {
        ...
        if (state) {
            ...
            if (state.messageId) {
                try {
                    await ctx.api.editMessageText(chatId, state.messageId, '⚠️ Request cancelled', { parse_mode: undefined });
                }
                catch (error) {
