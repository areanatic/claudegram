...
function shouldRetryWithProxy(errorMsg) {
...
function loadProxies() {
    if (!config.YTDLP_PROXY_LIST_PATH)
        return;
    try {
 ...
 */
async function runYtDlp(baseArgs, url, timeoutMs, onRetry) {
...
async function getVideoMeta(url, onRetry) {
    try {
...
// ── Audio Download ─────────────────────────────────────────────────
async function downloadAudio(url, outputDir, onRetry) {
...
// ── Video Download ─────────────────────────────────────────────────
async function downloadVideo(url, outputDir, onRetry) {
...
async function downloadSubtitles(url, outputDir, format, onRetry) {
    ...
    try {
        ...
        const files = fs.readdirSync(outputDir).filter(f => f.startsWith('subs.') && (f.endsWith('.srt') || f.endsWith('.vtt')));
        if (files.length === 0)
            return null;
        return path.join(outputDir, files[0]);
    }
...
// ── Audio Chunking ─────────────────────────────────────────────────
async function chunkAudio(inputPath, outputDir, chunkDurationSec) {
...
// ── Transcription ──────────────────────────────────────────────────
async function transcribeAudioFile(filePath, onProgress) {
...
export async function extractMedia(opts) {
        ...
        if (wantsVideo) {
            ...
            try {
                ...
                if (videoSize > TELEGRAM_VIDEO_MAX_MB * 1024 * 1024) {
                    result.warnings.push(`Video is ${(videoSize / 1024 / 1024).toFixed(1)}MB — exceeds Telegram's ${TELEGRAM_VIDEO_MAX_MB}MB limit.`);
                }
                else {
 ...
 */
export function cleanupExtractResult(result) {
    if (result._tempDir) {
    ...
    const paths = [result.audioPath, result.videoPath].filter(Boolean);
