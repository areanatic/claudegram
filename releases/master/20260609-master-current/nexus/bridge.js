...
function findAncestorWithMarkers(startDir) {
    ...
    while (true) {
        ...
        if (hasClaude && hasSoul && (hasPdRegistry || hasLegacyRegistry))
            return current;
...
export function findDefaultNexusRoot() {
    const candidates = [
...
export function detectNexusRoot(cwd) {
...
export function buildNexusBridgePrompt(cwd) {
    ...
    const registry = readJsonSafe(path.join(nexusRoot, '.claude', 'agents', 'pd', '_registry.json'))
        ?? readJsonSafe(path.join(nexusRoot, '00_NEXUS_CORE', 'agent_registry.json'));
