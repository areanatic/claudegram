...
import { recordInput, finalizeIfOpen } from '../../inbox/input-log.js';
/** chatId:messageId -> input_log row id. Bounded LRU-ish (oldest evicted). */
const rowIdByMessage = new Map();
const MAX_TRACKED = 500;
function trackKey(chatId, messageId) {
    return `${chatId}:${messageId}`;
}
/** Resolve the input_log row id for a received message, if one was recorded. */
export function getInputLogRowId(chatId, messageId) {
...
export function forgetInputLogRowId(chatId, messageId) {
    if (chatId == null || messageId == null)
        return;
...
function classify(ctx) {
    ...
    if (!msg)
        return null;
    if (msg.voice)
        return { type: 'voice', text: null, fileId: msg.voice.file_id };
    if (msg.audio)
        return { type: 'audio', text: null, fileId: msg.audio.file_id };
    if (msg.photo && msg.photo.length > 0) {
        return { type: 'photo', text: msg.caption ?? null, fileId: msg.photo[msg.photo.length - 1].file_id };
    }
    if (msg.document)
        return { type: 'document', text: msg.caption ?? null, fileId: msg.document.file_id };
    if (typeof msg.text === 'string')
        return { type: 'text', text: msg.text, fileId: null };
    return null;
}
export async function inputLogMiddleware(ctx, next) {
    let recordedRowId = null;
    try {
        ...
        const isCommand = classified?.type === 'text' &&
...
//# sourceMappingURL=input-log.middleware.js.map