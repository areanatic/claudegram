...
import { createInboxEntry, saveMetadata, isAllowedMimeType, formatFileSize, } from '../../inbox/inbox.js';
...
// ── Helpers ──────────────────────────────────────────────────────────
async function downloadTelegramDocument(ctx, fileId, destPath, fileSizeMB) {
...
export async function handleDocument(ctx) {
    ...
    if (!isAllowedMimeType(mimeType)) {
        ...
        await ctx.reply(`File type not supported: \`${esc(mimeType || 'unknown')}\``, { parse_mode: 'MarkdownV2' });
        return;
    }
    // ── Validate file size ──
    ...
    if (fileSizeMB > maxSizeMB) {
        await ctx.reply(`File too large \\(${esc(fileSizeMB.toFixed(1))}MB\\)\\.\nMax: ${esc(String(maxSizeMB))}MB\\.`, { parse_mode: 'MarkdownV2' });
        return;
    }
    // ── Prepare inbox entry ──
    ...
    try {
        ...
        await ctx.reply(`Failed to save document: ${esc(errorMessage)}`, { parse_mode: 'MarkdownV2' });
    }
}
// ── Batch Logic ──────────────────────────────────────────────────────
/**
 ...
 */
async function handleBatchOrImmediate(ctx, sessionKey, metadata) {
 ...
 */
async function sendSingleFileConfirmation(ctx, sessionKey, metadata) {
                ...
                if (getStreamingMode() === 'streaming') {
                    ...
                    try {
                        ...
                        if (error instanceof StaleTurnError)
                            throw error; // bubble to outer (tagged there)
 ...
 */
async function sendBatchConfirmation(ctx, sessionKey, files) {
...
export function clearAllBatchTimers() {
