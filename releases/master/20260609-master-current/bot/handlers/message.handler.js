import { sendToAgent, sendLoopToAgent, clearConversation, CLAUDE_CANCEL_SENTINEL_TEXT, StaleTurnError, assertTurnIsCurrent, isQuiet, } from '../../claude/agent.js';
...
import { queueRequest, isProcessing, getQueuePosition, setAbortController, cancelRequest, clearQueue, QueueFailsafeTimeoutError, QueueWaitTimeoutError, } from '../../claude/request-queue.js';
...
import { createRequestContext, disposeRequestContext, markSuccess, markCancelled, HandlerState, } from '../../handler/request-context.js';
 ...
 */
function buildContextCallbacks(ctx) {
    const onLongRunning = async (reqCtx) => {
        ...
        if (isQuiet(reqCtx.sessionKey))
            return;
        try {
            await ctx.reply(msg, { parse_mode: undefined });
        }
        catch (err) {
            console.debug(`[RequestContext ${reqCtx.requestId}] long-running notify failed:`, err);
        }
    };
    const onHardCap = async (reqCtx) => {
        ...
        try {
            await ctx.reply(`⏱ Timeout: Keine Antwort nach ${minutes} Min. Bitte nochmal senden.`, { parse_mode: undefined });
        }
        catch (err) {
            console.debug(`[RequestContext ${reqCtx.requestId}] hard-cap notify failed:`, err);
        }
    };
    return { onLongRunning, onHardCap };
}
function getAutoVRedditUrl(text) {
...
export async function handleMessage(ctx) {
    ...
    if (!keyInfo || !text || !messageId || !messageDate)
        return;
    ...
    if (!session) {
        await ctx.reply('⚠️ No project set\\.\n\nUse `/project` to open a project first\\.', { parse_mode: 'MarkdownV2' });
        return;
    }
    ...
    try {
        ...
        if (error instanceof QueueFailsafeTimeoutError) {
            console.log(`[handleMessage] swallowing QueueFailsafeTimeoutError for ${sessionKey} ` +
        ...
        if (error instanceof QueueWaitTimeoutError) {
            console.log(`[handleMessage] queue-wait timeout for ${sessionKey} after ${Math.round(error.waitedMs / 1000)}s`);
            markDropped(inputLogRowId, 'queue_wait_timeout');
            try {
                await ctx.reply('⏱ Timeout: Deine Anfrage hat zu lange in der Warteschlange gewartet. Bitte nochmal senden.', { parse_mode: undefined });
            }
            catch { /* best-effort */ }
            forgetInputLogRowId(chatId, messageId);
            return;
...
async function handleProjectReply(ctx, sessionKey, projectPath) {
    ...
    if (!isPathWithinRoot(workspaceRoot, resolvedPath)) {
        await ctx.reply(`❌ Path must be within workspace root: \`${esc(workspaceRoot)}\``, { parse_mode: 'MarkdownV2' });
        return;
    }
    // Check if exists
    if (!fs.existsSync(resolvedPath)) {
        await ctx.reply(`❌ Path not found: \`${esc(resolvedPath)}\`\n\nPlease check the path and try again\\.`, { parse_mode: 'MarkdownV2' });
        return;
    }
    // Check if directory
    if (!fs.statSync(resolvedPath).isDirectory()) {
        await ctx.reply(`❌ Not a directory: \`${esc(resolvedPath)}\``, { parse_mode: 'MarkdownV2' });
        return;
    }
    // Set the project
...
async function handleFileReply(ctx, sessionKey, filePath) {
    ...
    if (!isPathWithinRoot(workspaceRoot, fullPath)) {
        await ctx.reply(`❌ File path must be within workspace root: \`${esc(workspaceRoot)}\``, { parse_mode: 'MarkdownV2' });
        return;
    }
    if (!fs.existsSync(fullPath)) {
        await ctx.reply(`❌ File not found: \`${esc(trimmedPath)}\``, { parse_mode: 'MarkdownV2' });
        return;
    }
    if (fs.statSync(fullPath).isDirectory()) {
        await ctx.reply(`❌ That's a directory, not a file: \`${esc(trimmedPath)}\``, { parse_mode: 'MarkdownV2' });
        return;
    }
    const success = await messageSender.sendDocument(ctx, fullPath, `📎 ${path.basename(fullPath)}`);
    if (!success) {
        await ctx.reply('❌ Failed to send file\\. It may be too large \\(\\>50MB\\) or inaccessible\\.', { parse_mode: 'MarkdownV2' });
    }
}
/**
 ...
 */
export async function handleAgentReply(ctx, sessionKey, input, mode) {
    const session = sessionManager.getSession(sessionKey);
    if (!session) {
        await ctx.reply('⚠️ No project set\\.\n\nIf the bot restarted, use `/continue` or `/resume` to restore your last session\\.\nOr use `/project` to open a project first\\.', { parse_mode: 'MarkdownV2' });
        return;
    ...
    if (!trimmedInput) {
        await ctx.reply('❌ Please provide a description\\.', { parse_mode: 'MarkdownV2' });
        return;
    }
    try {
        await queueRequest(sessionKey, trimmedInput, async (turnEpoch) => {
            ...
            const origin = mode; // 'plan' | 'explore' | 'loop'
            ...
            try {
                ...
                try {
                    ...
                    const isAbort = innerErr instanceof Error &&
                    ...
                    if (!markCancelled(reqCtx, isAbort ? 'user-cancel' : 'system')) {
                        ...
                        console.log(`[RequestContext ${reqCtx.requestId}] error after finalize discarded: ` +
                ...
                if (isCancelSentinel || wasAlreadyCancelled) {
                    ...
                    console.log(`[RequestContext ${reqCtx.requestId}] ${mode} cancel-sentinel routed via ` +
                ...
                if (!markSuccess(reqCtx)) {
                    console.log(`[RequestContext ${reqCtx.requestId}] late ${mode} response discarded ` +
        ...
        if (error instanceof QueueFailsafeTimeoutError) {
            console.log(`[handleAgentReply ${mode}] swallowing QueueFailsafeTimeoutError ` +
        ...
        if (error instanceof QueueWaitTimeoutError) {
            console.log(`[handleAgentReply ${mode}] queue-wait timeout for ${sessionKey} after ` +
                `${Math.round(error.waitedMs / 1000)}s`);
            try {
                await ctx.reply('⏱ Timeout: Deine Anfrage hat zu lange in der Warteschlange gewartet. Bitte nochmal senden.', { parse_mode: undefined });
            }
            catch { /* best-effort */ }
...
async function handleTelegraphReply(ctx, sessionKey, filePath) {
    const trimmedPath = filePath.trim();
    const session = sessionManager.getSession(sessionKey);
    if (!session) {
        await ctx.reply('⚠️ No project set\\.\n\nIf the bot restarted, use `/continue` or `/resume` to restore your last session\\.\nOr use `/project` to open a project first\\.', { parse_mode: 'MarkdownV2' });
    ...
    if (ext !== '.md' && ext !== '.markdown') {
        await ctx.reply('⚠️ Telegraph works best with Markdown files \\(\\.md\\)', { parse_mode: 'MarkdownV2' });
    }
    await ctx.reply('📤 Creating Telegraph page\\.\\.\\.', { parse_mode: 'MarkdownV2' });
    const pageUrl = await createTelegraphFromFile(fullPath);
    if (pageUrl) {
        const fileName = path.basename(fullPath);
        await ctx.reply(`📄 *${esc(fileName)}*\n\n[Open in Instant View](${esc(pageUrl)})`, { parse_mode: 'MarkdownV2' });
    }
    else {
        await ctx.reply('❌ Failed to create Telegraph page\\.', { parse_mode: 'MarkdownV2' });
    }
}
async function handleStreamingResponse(ctx, sessionKey, message, turnEpoch, inputLogRowId) {
    ...
    try {
        ...
        if (isCancelSentinel || wasAlreadyCancelled) {
            ...
            markCancelled(reqCtx, 'user-cancel');
            console.log(`[RequestContext ${reqCtx.requestId}] cancel-sentinel routed via ` +
                `cancelStreaming (state=${reqCtx.state} sentinel=${isCancelSentinel})`);
            try {
                await messageSender.cancelStreaming(ctx);
            }
        ...
        if (!markSuccess(reqCtx)) {
            ...
            console.log(`[RequestContext ${reqCtx.requestId}] late agent response discarded ` +
                `(state=${reqCtx.state} reason=${reqCtx.cancelReason})`);
...
async function handleWaitResponse(ctx, sessionKey, chatId, message, turnEpoch, inputLogRowId) {
    ...
    try {
        ...
        if (isCancelSentinel || wasAlreadyCancelled) {
            markCancelled(reqCtx, 'user-cancel');
            console.log(`[RequestContext ${reqCtx.requestId}] wait cancel-sentinel suppressed ` +
