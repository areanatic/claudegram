...
function seed() {
    ...
    END;
  `);
    const ins = db.prepare(`INSERT INTO memories (type, content, source, project, privacy, score, archived, created_at)
...
const { searchMemoryReadOnly, recentMemoriesReadOnly, readMemoryPolicyFromEnv, DEFAULT_TRUSTED_PRIVATE_SOURCES, } = mem;
const has = (rows, m) => rows.some(r => r.content.includes(m));
const markers = (rows) => ['PUB ', 'PRIV_CUSTOM', 'PRIV_DEFAULT', 'PUB_ARCHIVED', 'PUB_ZERO'].filter(m => has(rows, m)).sort();
// ── 1. Custom CSV override re-shapes the trusted set ───────────────────────────
{
    // Only 'my-custom-src' is trusted now; 'omi' (a DEFAULT) is NOT.
    const pol = { scope: 'self_private', trustedPrivateSources: ['my-custom-src'] };
...
// ── 2. env override path (readMemoryPolicyFromEnv parses NEXUS_TRUSTED_PRIVATE_SOURCES) ─
{
    ...
    check(envPol.trustedPrivateSources.includes('my-custom-src') && envPol.trustedPrivateSources.includes('omi'), 'env: both distinct trimmed sources retained');
    const rows = searchMemoryReadOnly(KW, 20); // no explicit policy → uses env
    check(has(rows, 'PRIV_CUSTOM') && has(rows, 'PRIV_DEFAULT'), 'env: both env-trusted private rows visible via env-derived policy');
    delete process.env.NEXUS_MEMORY_SCOPE;
    delete process.env.NEXUS_TRUSTED_PRIVATE_SOURCES;
}
// ── 3. EMPTY trusted list under self_private → behaves like public (no leak) ────
{
    const pol = { scope: 'self_private', trustedPrivateSources: [] };
    const rows = searchMemoryReadOnly(KW, 20, undefined, { policy: pol });
    check(!has(rows, 'PRIV_CUSTOM') && !has(rows, 'PRIV_DEFAULT'), 'empty-trusted self_private: NO private row leaks (falls through to public branch)');
    ...
    check(!has(rec, 'PRIV_CUSTOM') && !has(rec, 'PRIV_DEFAULT'), 'empty-trusted self_private (recent): no private leak');
}
// ── 4. CSV with an INVALID slug → rejected, valid neighbours survive ───────────
{
    ...
    check(p.trustedPrivateSources.includes('my-custom-src') && p.trustedPrivateSources.includes('omi'), `invalid-slug: valid neighbours survived (got ${p.trustedPrivateSources})`);
...
// ── 5. >32 trusted sources → MAX_TRUSTED_SOURCES cap at 32 ──────────────────────
{
    ...
    check(p.trustedPrivateSources[0] === 'src-0' && p.trustedPrivateSources[31] === 'src-31', 'max-cap: first 32 kept in order, 33rd+ dropped');
    delete process.env.NEXUS_TRUSTED_PRIVATE_SOURCES;
}
// ── 6. archived rows excluded from recent even under operator_all ───────────────
{
    const trusted = [...DEFAULT_TRUSTED_PRIVATE_SOURCES];
    const opAll = { scope: 'operator_all', trustedPrivateSources: trusted };
...
{
    const trusted = [...DEFAULT_TRUSTED_PRIVATE_SOURCES];
    const pub = { scope: 'public', trustedPrivateSources: trusted };
...
// ── 8. project filter never bypasses privacy (recent path, family project) ──────
{
    ...
    check(!has(recNexus, 'PRIV_CUSTOM') && !has(recNexus, 'PRIV_DEFAULT'), 'recent/public+project=nexus: project filter does NOT bypass privacy');
}
try {
...
//# sourceMappingURL=privacy-matrix.edge.test.js.map