...
function buildFixture(rows) {
    ...
    const tx = db.transaction((rs) => {
        for (const r of rs)
            for (let i = 0; i < r.n; i++)
                ins.run(r.source, r.privacy, `c${i}`);
    });
    tx(rows);
    db.close();
}
...
//     so a future typo or a new writer added outside the list is caught here.
{
    ...
    check(actual.length === expected.length && expected.every((s, i) => actual[i] === s), `(6) OMI_WRITER_SOURCES inventory unchanged (got [${actual.join(', ')}])`);
}
// cleanup
try {
...
//# sourceMappingURL=omi-bridge-watcher.postcondition.test.js.map