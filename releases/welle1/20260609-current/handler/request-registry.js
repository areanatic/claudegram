 ...
 */
import { HandlerState } from './request-context.js';
...
const activeContextsBySession = new Map();
/** requestId → context (global lookup, e.g. for /health summary). */
const allContextsById = new Map();
export function registerRequestContext(ctx) {
...
export function unregisterRequestContext(ctx) {
 ...
 */
export function getActiveQueueLength(sessionKey) {
 ...
 */
export function getActiveContextsForSession(sessionKey) {
    const list = activeContextsBySession.get(sessionKey);
    if (!list || list.length === 0)
        return [];
    // Filter out terminal contexts to avoid double-finalize attempts.
    return list.filter((c) => c.state === HandlerState.WAITING ||
 ...
 */
export function getEarliestDeadline(sessionKey) {
    const list = activeContextsBySession.get(sessionKey);
    if (!list || list.length === 0)
        return undefined;
    let earliest = list[0].deadline_ms;
    for (let i = 1; i < list.length; i++) {
        if (list[i].deadline_ms < earliest)
            earliest = list[i].deadline_ms;
    }
    return earliest;
}
 ...
 */
export function sweepTerminalContexts(now = Date.now()) {
    let removed = 0;
    ...
    const candidates = [];
    for (const ctx of allContextsById.values()) {
        if (ctx.state === HandlerState.RESPONDED ||
    ...
    for (const ctx of candidates) {
        if (ctx.state !== HandlerState.RESPONDED &&
            ...
            console.warn(`[RequestRegistry sweep] dropping orphaned non-terminal ctx ` +
 ...
 */
export function startRegistrySweep(intervalMs = 60_000) {
    if (sweepTimer)
        return;
    sweepTimer = setInterval(() => {
        try {
            ...
            if (removed > 0) {
                console.log(`[RequestRegistry] periodic sweep removed ${removed} terminal/orphan ctx`);
            }
        }
        catch (err) {
...
export function stopRegistrySweep() {
    if (sweepTimer) {
