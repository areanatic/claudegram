...
export const DEFAULT_TRUSTED_PRIVATE_SOURCES = Object.freeze([
    'omi',
    'omi-bridge',
    'omi-bridge-task',
    'omi-synthesis',
...
function sanitizeTrustedSources(raw) {
    const out = [];
    for (const s of raw) {
        const t = (s || '').trim();
        if (!t)
        ...
        if (!out.includes(t))
            out.push(t);
        if (out.length >= MAX_TRUSTED_SOURCES)
            break;
    }
    return out;
}
...
export function readMemoryPolicyFromEnv(env = process.env) {
...
export function buildPrivacyClause(conn, policy, alias = 'm') {
 ...
 */
let privacyColumnAvailable = null;
function hasPrivacyColumn(conn) {
    if (privacyColumnAvailable !== null)
        return privacyColumnAvailable;
    try {
        const rows = conn.prepare(`PRAGMA table_info(memories)`).all();
...
let botColumnAvailable = null;
function hasBotColumn(conn) {
    if (botColumnAvailable !== null)
 ...
 */
function privacyClause(conn, includePrivate, alias = 'm') {
    if (!hasPrivacyColumn(conn))
        return '';
 ...
 */
export function searchMemory(query, limit = 5, project, includePrivate = false) {
    const conn = getDb();
    if (!conn)
        return [];
    try {
 ...
 */
export function recentMemories(limit = 5, project, includePrivate = false) {
    const conn = getDb();
    if (!conn)
        return [];
    try {
 ...
 */
export function saveMemory(content, type = 'episodic', project, tags, source = 'nexusgram', privacy = 'public', 
...
bot = botId()) {
    ...
    try {
        ...
        if (hasPrivacyColumn(conn)) {
          ...
          VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        `);
                const result = stmt.run(type, content, source, project || null, tags || null, decayRate, privacy, bot || null);
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?)
      `);
            const result = stmt.run(type, content, source, project || null, tags || null, decayRate, privacy);
            return Number(result.lastInsertRowid);
        }
 ...
 */
export function injectContext(query, project, includePrivate = false) {
...
export function searchMemoryReadOnly(query, limit = 5, project, options = {}) {
    ...
    `);
        ...
        let rows = buildStmt().all(...buildParams(phraseQuery));
        ...
        if (rows.length === 0) {
            const tokenQuery = query
                ...
                .join(' OR ');
            if (tokenQuery) {
                rows = buildStmt().all(...buildParams(tokenQuery));
            }
        }
        if (auditEnabled) {
            ...
            const breakdown = Array.from(sourceCounts.entries())
                .map(([s, n]) => `${s}:${n}`)
                .join(',') || 'none';
            console.error(`[NexusMemory/MCP] scope=${policy.scope} hits=${rows.length} ` +
 ...
 */
export function recentMemoriesReadOnly(limit = 5, project, options = {}) {
    const clampedLimit = Math.max(1, Math.min(20, Math.floor(limit)));
    const policy = options.policy ?? readMemoryPolicyFromEnv();
    let conn = null;
    try {
...
export function searchTasks(opts = {}) {
    ...
    try {
        ...
        const where = ['mt.deleted=0', 'mt.archived=0'];
        const params = [];
        if (status === 'open') {
    ...
    `;
        ...
        return rows.map(r => {
            ...
            if (r.created_at_utc) {
                ...
                if (!Number.isNaN(t))
                    age = Math.floor((now - t) / 86_400_000);
            }
            return {
...
export function resolvePerson(query) {
    ...
    try {
        ...
        const exactAliases = conn.prepare(`SELECT DISTINCT p.person_id, p.canonical_name, p.status, p.mention_count
       FROM person_aliases a JOIN persons p ON p.person_id = a.person_id
       WHERE a.alias_normalized = ? ORDER BY p.mention_count DESC`).all(norm);
        if (exactAliases.length === 1) {
            const r = exactAliases[0];
            return { personId: r.person_id, canonicalName: r.canonical_name, status: r.status, ambiguous: [], suggestions: [] };
        }
        if (exactAliases.length > 1) {
            return {
                personId: exactAliases[0].person_id,
                canonicalName: exactAliases[0].canonical_name,
                status: exactAliases[0].status,
        ...
        const canonicals = conn.prepare(`SELECT person_id, canonical_name, status, mention_count FROM persons
       WHERE canonical_name = ? COLLATE NOCASE ORDER BY mention_count DESC`).all(trimmed);
        if (canonicals.length >= 1) {
            const r = canonicals[0];
            return { personId: r.person_id, canonicalName: r.canonical_name, status: r.status,
        ...
        const aliasSugg = conn.prepare(`SELECT DISTINCT p.canonical_name AS label, p.mention_count AS mention_count
       FROM person_aliases a JOIN persons p ON p.person_id = a.person_id
       WHERE a.alias_normalized LIKE ? OR a.alias_normalized LIKE ?
       ORDER BY p.mention_count DESC LIMIT 5`).all(prefixLike, containsLike);
        const unresolvedSugg = conn.prepare(`SELECT canonical_text AS label, COUNT(*) AS mention_count FROM entity_mentions
       WHERE resolved_person_id IS NULL AND entity_kind = 'person'
         AND (LOWER(canonical_text) LIKE ? OR LOWER(canonical_text) LIKE ?)
       GROUP BY canonical_text ORDER BY mention_count DESC LIMIT 5`).all(prefixLike, containsLike);
        return {
            ...empty,
            suggestions: [
                ...aliasSugg.map(s => ({ ...s, source: 'alias' })),
                ...unresolvedSugg.map(s => ({ ...s, source: 'unresolved' })),
            ].slice(0, 8),
        };
    }
    catch (err) {
...
export function searchPersonTimeline(opts) {
    const policy = opts.policy ?? readMemoryPolicyFromEnv();
    const empty = (resolution, scope_denied) => ({ resolution, hits: [], more_available: false, scope_denied });
    ...
    `;
        ...
        if (process.env.NEXUS_MEMORY_AUDIT === '1') {
            ...
            for (const r of sliced)
                counts.set(r.source_kind, (counts.get(r.source_kind) ?? 0) + 1);
...
export function searchEntities(opts = {}) {
    const policy = opts.policy ?? readMemoryPolicyFromEnv();
    const empty = (scope_denied) => ({ hits: [], more_available: false, scope_denied, top_entities_in_window: [] });
    ...
    try {
        ...
        const fOmiMem = (() => {
            ...
            const params = [];
            if (opts.from) {
                clauses.push('omi_mem.created_at_utc >= ?');
                params.push(opts.from);
            }
            if (opts.to) {
                clauses.push('omi_mem.created_at_utc <= ?');
        ...
        const fSeg = (() => {
            ...
            const params = [];
            if (opts.from) {
                clauses.push('omi_sess.started_at_utc >= ?');
                params.push(opts.from);
            }
            if (opts.to) {
                clauses.push('omi_sess.started_at_utc <= ?');
    ...
    `;
      ...
      `;
            ...
            const aggRows = conn.prepare(aggSql).all(...aggParams);
            topEntities = aggRows;
        }
        if (process.env.NEXUS_MEMORY_AUDIT === '1') {
