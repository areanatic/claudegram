...
import { getCaptureByMessage, appendTags, updateCaptureProcessed, } from './captures-db.js';
import { extractMedia, cleanupExtractResult, } from '../media/extract.js';
import { transcribeFile, downloadTelegramAudio, } from '../audio/transcribe.js';
const TRIGGER_REGEX = /^\s*(wichtig|important|deep|tief|interessant|merken|merk\s*dir|save\s*deep|kuck\s*genauer|schau\s*genauer|h[öo]r\s*dir\s*das\s*an|schau\s*dir\s*das\s*an)\b/i;
function botId() {
    return (config.BOT_NAME || 'Nexusgram').toLowerCase().replace(/\s+/g, '-');
}
function userExtraText(reply) {
    return reply.replace(TRIGGER_REGEX, '').replace(/^[\s,:.\-—!?]+/, '').trim();
}
/**
 * Returns true if `text` looks like a "wichtig"-style trigger.
 */
export function isWichtigTrigger(text) {
 ...
 */
export async function handleWichtigReply(ctx) {
    const reply = ctx.message?.text;
    const replyTo = ctx.message?.reply_to_message;
    const chatId = ctx.chat?.id;
    if (!reply || !replyTo || !chatId)
        return false;
    if (!isWichtigTrigger(reply))
        return false;
    ...
    try {
        ...
        await ctx.reply(`ℹ️ Capture #${cap.id} (${cap.capture_type}) — Deep-Pass für diesen Typ kommt in Phase 2 (Vision/PDF). Tag "wichtig" ist gespeichert.`);
        return true;
    }
    catch (err) {
...
async function deepVoice(ctx, captureId) {
    const cap = getCaptureByMessage(String(ctx.chat?.id ?? ''), ctx.message?.reply_to_message?.message_id ?? 0, botId());
    if (!cap?.id || cap.id !== captureId) {
        throw new Error('Capture lookup mismatch');
    }
    const fileId = await getFileIdFromCapture(captureId);
    if (!fileId)
        throw new Error('Kein telegram_file_id gespeichert');
    const file = await ctx.api.getFile(fileId);
    if (!file.file_path)
        throw new Error('Telegram getFile lieferte kein file_path');
...
function dbConn() {
    if (_db)
        return _db;
    try {
        _db = new Database('/Volumes/AstronOne/NEXUS_miniM_13-03-26/.nexus-memory/memory.db', { readonly: true });
        return _db;
    }
    catch (err) {
...
async function getFileIdFromCapture(id) {
    const conn = dbConn();
    if (!conn)
        return null;
    const row = conn
...
//# sourceMappingURL=wichtig-trigger.js.map