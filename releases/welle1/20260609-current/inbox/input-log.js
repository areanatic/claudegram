...
function getDb() {
    ...
    `);
        ...
        const cols = conn.prepare('PRAGMA table_info(input_log)').all().map((c) => c.name);
      ...
      `);
        ...
        conn.exec('CREATE UNIQUE INDEX IF NOT EXISTS idx_input_log_msg ON input_log(chat_id, message_id)');
      ...
      `);
            ...
            if (!alreadyApplied) {
                const stale = conn
                    .prepare("SELECT COUNT(*) AS n FROM input_log WHERE privacy IS NULL OR privacy = '' OR privacy = 'private'")
                    .get();
                const tx = conn.transaction(() => {
                    const result = conn
                        .prepare("UPDATE input_log SET privacy = 'public' WHERE privacy IS NULL OR privacy = '' OR privacy = 'private'")
                        .run();
                    conn.prepare(`INSERT INTO input_log_migrations (name) VALUES (?)`).run(migrationName);
                    return result.changes;
                });
                const changes = tx();
                console.log(`[InputLog] Stage 2d one-time backfill applied: ${changes} historical row(s) → privacy='public' (stale=${stale.n}, marker '${migrationName}' stored)`);
            }
            else {
                console.log('[InputLog] Stage 2d backfill already applied — skipping (legitimate /private rows preserved across restart)');
            }
        }
        catch (backfillErr) {
 ...
 */
export function recordInput(opts) {
    const conn = getDb();
    if (!conn)
        return null;
    try {
    ...
    `);
        const info = stmt.run(now, opts.messageId ?? null, opts.chatId, opts.sessionKey, opts.inputType, opts.rawContent ?? null, opts.fileId ?? null, now, privacy);
        if (info.changes > 0) {
        ...
        const existing = conn
            ...
            .get(opts.chatId, opts.messageId ?? null);
...
export function markProcessing(rowId) {
...
export function markDone(rowId) {
 ...
 */
export function markHandledNoAgent(rowId, reason) {
 ...
 */
export function markDoneRecovered(rowId, reason) {
 ...
 */
export function getRowExecutionState(rowId) {
    const conn = getDb();
    if (!conn || rowId == null)
        return null;
    try {
        const row = conn
            .prepare(`SELECT side_effect_tool_started_at AS se, resume_attempts AS attempts, received_at AS receivedAt
 ...
 */
export function hasLaterOpenVoiceRow(sessionKey, excludeRowId, receivedAt) {
    const conn = getDb();
    if (!conn)
        return false;
    try {
        const row = conn
            .prepare(`SELECT 1 FROM input_log
...
export function incrementResumeAttempt(rowId) {
    const conn = getDb();
    if (!conn || rowId == null)
        return;
    try {
 ...
 */
export function markSideEffectStarted(rowId, toolName) {
    const conn = getDb();
    if (!conn || rowId == null)
        return;
    try {
...
export function attachContent(rowId, content) {
    const conn = getDb();
    if (!conn || rowId == null)
        return;
    try {
...
function updateStatus(rowId, status, extra = {}) {
    const conn = getDb();
    if (!conn || rowId == null)
        return;
    try {
 ...
 */
export function finalizeIfOpen(rowId) {
    const conn = getDb();
    if (!conn || rowId == null)
        return;
    try {
 ...
 */
export function countHandlerNoFinalize() {
    const conn = getDb();
    if (!conn)
        return -1;
    try {
 ...
 */
export function recoverOrphanedInputs() {
    const conn = getDb();
    if (!conn)
    ...
    try {
        ...
        const recentOrphans = conn
            .prepare(`SELECT chat_id AS chatId, session_key AS sessionKey, input_type AS inputType,
                raw_content AS rawContent, received_at AS receivedAt, privacy
           FROM input_log
          WHERE status IN ('received', 'processing')
            AND received_at <= ? AND received_at >= ?`)
            .all(cutoff, recentCutoff);
        const info = conn
            .prepare(`UPDATE input_log
          ...
          WHERE status IN ('received', 'processing') AND received_at <= ?`)
            .run(cutoff, cutoff);
        if (info.changes > 0) {
            console.log(`[InputLog] boot-recovery: marked ${info.changes} orphaned input(s) dropped ` +
...
export function clampBootCap(raw) {
 ...
 */
export function claimResumableOrphans() {
    const conn = getDb();
    if (!conn)
    ...
    try {
        ...
        const claim = conn.transaction(() => {
            ...
            const eligible = conn
                .prepare(`SELECT id, chat_id AS chatId, session_key AS sessionKey,
        ...
        if (result.resumable.length > 0 || result.recovered > 0) {
            console.log(`[InputLog] claimResumableOrphans: ${result.resumable.length} claimed for replay, ` +
 ...
 */
export function hasNewerDuplicate(sessionKey, rawContent, excludeRowId, afterReceivedAt) {
    const conn = getDb();
    if (!conn)
        return false;
    try {
...
export function searchInputLog(opts) {
  ...
  `;
    try {
        const params = { match: phrase, limit };
        if (opts.sessionKey)
            params.sessionKey = opts.sessionKey;
        if (since)
            params.since = since;
        let rows = conn.prepare(sql).all(params);
 ...
 */
export function getLatestInputLog(sessionKey, limit = 5, opts = {}) {
    const conn = getDb();
    if (!conn)
        return [];
    ...
    try {
        const sql = excludeRowId != null
          ...
          ORDER BY received_at DESC
          LIMIT ?`;
        const params = excludeRowId != null
 ...
 * been marked).
 */
export function ensureInputLogInitialized() {
    if (db || initFailed)
        return;
    getDb();
}
