...
import { sendToAgent, CLAUDE_CANCEL_SENTINEL_TEXT, StaleTurnError, assertTurnIsCurrent, } from '../../claude/agent.js';
...
import { queueRequest, isProcessing, getQueuePosition, setAbortController, gracefulCancel, } from '../../claude/request-queue.js';
...
export async function handleVoice(ctx) {
    ...
    if (!keyInfo || !messageId || !messageDate || !voice)
        return;
    ...
    if (fileSizeMB > config.VOICE_MAX_FILE_SIZE_MB) {
        await ctx.reply(`❌ Voice note too large \\(${esc(fileSizeMB.toFixed(1))}MB\\)\\.\n\nPlease send shorter notes \\(max ${config.VOICE_MAX_FILE_SIZE_MB}MB\\)\\.`, { parse_mode: 'MarkdownV2' });
        return;
    }
    // Acknowledge receipt
    ...
    try {
        ...
        if (qualityFlag !== 'ok' || !languageOk) {
            ...
            console.warn(`[Voice] transcript rejected: lang=${detectedLanguage} languageOk=${languageOk} ` +
        ...
        if (voiceActive) {
            ...
            try {
                await ctx.api.editMessageText(chatId, ackMsg.message_id, '🎤 Transcript received\\.', { parse_mode: 'MarkdownV2' });
            }
            catch {
                try {
                    await ctx.api.deleteMessage(chatId, ackMsg.message_id);
        ...
        if (isHardTimeout) {
            ...
            if (hardCancelDrain) {
                try {
                    await hardCancelDrain;
                }
                catch { /* best-effort — drain errors don't block */ }
            }
        ...
        try {
            ...
            await ctx.reply(plainReply ? errorMessage : `❌ Voice error: ${esc(errorMessage)}`, plainReply ? { parse_mode: undefined } : { parse_mode: 'MarkdownV2' });
        }
    }
    finally {
 ...
 */
async function handleTranscribeOnly(ctx, chatId, messageId, voice, inputLogRowId) {
    const ackMsg = await ctx.reply('🎤 Transcribing...', { parse_mode: undefined });
    let tempFilePath = null;
    try {
        const file = await ctx.api.getFile(voice.file_id);
